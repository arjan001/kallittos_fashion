...
import { deliveryLocations as initialLocations, formatPrice } from "@/lib/data"
...
export function AdminDelivery() {
  ...
  const handleSave = () => {
    ...
    if (editId) setLocations((prev) => prev.map((l) => (l.id === editId ? loc : l)))
                ...
                <tr key={loc.id} className="hover:bg-secondary/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => setLocations((prev) => prev.filter((l) => l.id !== loc.id))}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
